...
def test_getitem():
    ...
    assert L[-3:-1:1] == [2, 3]
    # slice(-3,-1,-1) --> slice(1,3,1)  --> for i in range(1,3,1)

    assert L[0:0] == []  # for i in range(0,0) ...
    assert L[2:2] == []  # for i in range(2,2)

    assert L[:10] == [1, 2, 3, 4]
    # slice(None,10,None) --> slice(0,sys.maxsize,1) --> for i in range(0,sys.maxsize,1):...
    try:
        assert L[10] == []
    except IndexError:
        assert True
...
def test_setitem():
    ...
    L = [1, 1, 1, 1, 1, 1]
    L[None::-2] = [2, 3, 4]
    # SLICE: for new_index, position in enumerate(reversed(range(start,end,-2)): REPLACE L[position] WITH NEW[new_index]

...
| Version    | Change                                              |
|:-----------|-----------------------------------------------------|
| 2022.11.18 | `copy`, `filter`, `sort`, `any`, `all` methods now properly respects the table subclass.<br>Filter for tables with under `SINGLE_PROCESSING_LIMIT` rows will run on same process to reduce overhead.<br>Errors within child processes now properly propogate to parent. |

...
| Version    | Change                                              |
|:-----------|-----------------------------------------------------|
|  MASTER    | Bug: A column with `1,None,2` would be written to csv & tsv as `"1,None,2"`. Now it is written `"1,,2"` where None means absent.<br> Feature: `Table.reset_storage(include_imports=True)` now allows the user to reset the storage but exclude any imported files by setting `include_imports=False` during `Table.reset(...)`.|
